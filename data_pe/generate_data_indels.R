...
for(file.bam in bamlist){
  ...
  param <- ScanBamParam(flag=scanBamFlag(isUnmappedQuery=FALSE,isDuplicate=NA,isPaired = TRUE),what=c("qname","pos", "cigar","mapq"))
  ...
  indels <- mclapply(seq_len(nrow(di.reads)),FUN = getIndels,di.reads,mc.cores = 2)
